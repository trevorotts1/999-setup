...
import { evaluateReadiness, finalize, REPORT_SCHEMA, FIX_ID, REPORT_MARKER } from "../codesign-readiness.mjs";
...
test("fully passing environment yields RELEASE_AUTHORIZED and exit 0", () => {
  const { report, exitCode } = evaluateReadiness({ dmgPath: "/fixture/Candice-Companion.dmg", deps: greenDeps() });
...
test("zero-identity keychain is BLOCKED with the Developer ID credential action", () => {
  const { report, exitCode } = evaluateReadiness({ dmgPath: "/fixture/Candice-Companion.dmg", deps: blockingDeps() });
