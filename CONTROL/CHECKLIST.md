...
- [ ] Existing-user update path: GitHub `main` update alone never installs a new desktop app; the existing-user flow (self-update → next invocation bootstrap installs/repairs missing components → stale skills refresh → fast health check after) works end-to-end (spec 21).
