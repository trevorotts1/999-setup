...
**Master Spec:** `CANDICE_COMPANION_AI_IMPLEMENTATION_SPEC_V6_FINAL_LOCKED.md` — source at `~/Downloads/CANDICE_COMPANION_AI_IMPLEMENTATION_SPEC_V6_FINAL_LOCKED.md`; canonicalization to `SPEC/MASTER-SPEC-2026-08-21.md` per spec 0J is owned by the apparatus-baseline run (WR-002) and was pending at this file's creation.
...
**Updated:** 2026-08-21 (WR-002 apparatus baseline)
...
|---|---|---|---|---|---|---|---|---|---|
| WR-001 | `candice-bootstrap-audit-wf_bb855713-af9.js` | Bootstrap/audit: fetch/refresh `main`, inspect live repo, record SHA/versions, establish working branch/worktree | 5 | 0 | opus/max (Nine-router — currently DeepSeek V4 Flash) | — (audit run; no QC seats) | none (first run) | branch `candice/wr001-bootstrap`; worktree `worktrees/wr001-bootstrap` | COMPLETED 2026-08-21 — 5/5 OK; handle `wf_bb855713-af9` |
| WR-002 | `candice-apparatus-baseline` (this run, in flight) | Control apparatus baseline per 0J: TODO, CHECKLIST, LEDGER, EXECUTION-PLAN, SESSION-LOG, `project_state.json`, task-graph snapshot — reconcile against live repo | 5 | 5 | opus/max (Nine-router — currently DeepSeek V4 Flash) | sonnet/max (Nine-router — currently DeepSeek V4 Pro) | WR-001 | main checkout (no dedicated worktree yet; disjoint owned paths per unit — one file per builder unit); branch `candice/wr002-apparatus-baseline` planned | IN_FLIGHT 2026-08-21 |
| WR-003 | `candice-planning-architecture` (this run) | Planning/architecture: task graph, ownership map, acceptance criteria, launch board — plus QC units on those deliverables | 4 | 2 | opus/max (Nine-router — currently DeepSeek V4 Flash) | sonnet/max (Nine-router — currently DeepSeek V4 Pro) | WR-001, WR-002 | worktree `worktrees/wr001-bootstrap` base; owned units: task-graph / ownership-map / acceptance-criteria / launch-board + QC units on each | IN_FLIGHT 2026-08-21 — handle `wf_9529b3f1-4bb` |
| WR-004 | `candice-capacity` | Capacity/canaries per 0H: profiler result, opus route canary, sonnet route canary, max-thinking proof, capacity ledger, safe live width | 3 | 2 | opus/max (Nine-router — currently DeepSeek V4 Flash) | sonnet/max (Nine-router — currently DeepSeek V4 Pro) | WR-001 | capacity-ledger owned unit; canary runs on operator box | DISPATCHED_CONCURRENT 2026-08-21 — handle `wf_<pending>` |

**Launch summary (spec 0D example form):**

```text
CANDICE PARALLEL LAUNCH — apparatus baseline
Visible workflow runs: 2 (WR-001 completed, WR-002 in flight)
Builder seats: WR-002 = 5 Opus/max
QC seats: WR-002 = 5 Sonnet/max
Route check: per 0H — canaries run before implementation dispatch (capacity workflow)
Workflow visibility check: WR-001 handle wf_bb855713-af9 recorded; WR-002 handle in session task tracking
Blocked workflow runs: 0
Wave: 0 (apparatus) — implementation waves W1-W5 below
```

**Reserved naming:** subsequent runs take `WR-003`, `WR-004`, ... in launch order. Each row must carry a real returned handle before it counts as launched (0D visibility gate). Future rows are appended here by the conductor; never start a duplicate board document (0J).
...
The Capacity Ledger (`CAPACITY-LEDGER.md`) is the measured source. Before implementation dispatch the 0H gates require: current profiler result, opus route canary, sonnet route canary, max-thinking proof on both seats, task graph, board section, unique ownership, worktree plan, shared-file list, safe width, no padding.
