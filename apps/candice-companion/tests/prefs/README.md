...
7. Versioned JSON schema + migration registry; v1 fixtures preserved for the
   WS-34 v2 migration test; future versions preserved at their own version in
   memory and never rewritten by an older lane (save refused, disk untouched).
8. Voice-output ON/OFF persists independently of answer method (spec 5.2);
   all four voice/type combinations round-trip.
9. Spec-9 fields persist: volume, speech rate, text scale, reduced motion,
   companion position, last answer method (convenience, never a lock),
   optional last-used skill.
...
npx tsx --test tests/prefs/prefs.test.ts
```
...
installing the app's full toolchain.
