...
selftest() {
  ...
  # --- case 19: THE SNAPSHOT WITNESS (RC-29b). A project past step 6.5 (the
  ...
  mk_home "$T/c19"
  printf 'CLIENT_CAP=10\n' > "$T/c19/CAPACITY-LEDGER.md"
  set +e
  OUT="$(bash "$SELF" "$T/c19" 2>&1)"; RC=$?
  set -e
  local rc19a="$RC" alarm19a=0
  "$GREP" -q 'tasks-snapshot-absent' "$T/c19/CONTROL/LEDGER.md" 2>/dev/null && alarm19a=1
  set +e
  OUT="$(bash "$SELF" "$T/c19" 2>&1)"; RC=$?
  set -e
  ok=0
  if (( RC == 3 )) && (( alarm19a == 0 )) \
     ...
     && "$GREP" -q 'DRIFT-ALARM | tasks-snapshot-absent' "$T/c19/CONTROL/LEDGER.md" 2>/dev/null; then ok=1; fi
  report 19 "snapshot-absent-alarm" "$ok" "first tick rc=${rc19a} silent (alarm on ledger=${alarm19a}, want 0 — one tick of grace); second tick rc=${RC} (want 3) with DRIFT-ALARM | tasks-snapshot-absent and ACTION|write-task-snapshot"
  # the control: the snapshot present, the SAME project stays silent.
  bash "${SCRIPT_DIR}/anchor.sh" --write-tasks "$T/c19" >/dev/null 2>&1
  rm -f "$T/c19/CONTROL/.snapshot-missing"
  "$GREP" -v 'tasks-snapshot-absent' "$T/c19/CONTROL/LEDGER.md" > "$T/c19/CONTROL/LEDGER.md.clean" 2>/dev/null \
    && mv "$T/c19/CONTROL/LEDGER.md.clean" "$T/c19/CONTROL/LEDGER.md" || true
  set +e
  OUT="$(bash "$SELF" "$T/c19" 2>&1)"; RC=$?
  set -e
  ok=0
  if ! printf '%s' "$OUT" | "$GREP" -q 'tasks-snapshot-absent' \
     && ! "$GREP" -q 'tasks-snapshot-absent' "$T/c19/CONTROL/LEDGER.md" 2>/dev/null \
     && [[ ! -f "$T/c19/CONTROL/.snapshot-missing" ]]; then ok=1; fi
  report 19 "snapshot-present-control" "$ok" "rc=${RC}; no tasks-snapshot-absent on stdout or the ledger, and no .snapshot-missing flag file — the witness discriminates"
