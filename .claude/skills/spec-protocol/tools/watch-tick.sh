...
#  13  SPEECH — a drafted client message under CONTROL/.speech/ with no
#      SPEECH-CHECK line on the ledger -> exit 3, DRIFT-ALARM speech-unchecked
#  14  THE CONTROL FOR 13 — the same draft WITH its SPEECH-CHECK line -> exit 0
#      and no alarm. 13 and 14 differ by one ledger line, which is the whole
...
selftest() {
  ...
  # --- case 13: SPEECH. A client message was drafted for turn 07 and no
  ...
  mk_home "$T/c13"
  printf '%s | U-02 qc | qc | [opus x10] WF01 judge | run-013\n' "$(stamp 1)" > "$T/c13/CONTROL/dispatch-log.md"
  printf '%s | WF01 judge | U-02 | qc\n' "$(stamp 1)" > "$T/c13/CONTROL/HEARTBEAT.md"
  mkdir -p "$T/c13/CONTROL/.speech"
  printf 'Your website is live. Have a look and tell me what to change.\n' > "$T/c13/CONTROL/.speech/turn-07.txt"
  runw "$T/c13"
  ok=0
  if (( RC == 3 )) \
     ...
     && "$GREP" -q 'DRIFT-ALARM | speech-unchecked | drafts=1 unchecked=1' "$T/c13/CONTROL/LEDGER.md" 2>/dev/null; then ok=1; fi
  report 13 "speech-unchecked" "$ok" "rc=${RC} (want 3); DRIFT-ALARM | speech-unchecked written for turn-07.txt; ACTION|speech-check emitted; the S-CHECK line carries speech=unchecked(turn-07.txt)"

  # --- case 14: THE CONTROL FOR 13. Byte for byte the same fixture plus the
  ...
  mk_home "$T/c14"
  printf '%s | U-02 qc | qc | [opus x10] WF01 judge | run-014\n' "$(stamp 1)" > "$T/c14/CONTROL/dispatch-log.md"
  printf '%s | WF01 judge | U-02 | qc\n' "$(stamp 1)" > "$T/c14/CONTROL/HEARTBEAT.md"
  mkdir -p "$T/c14/CONTROL/.speech"
  printf 'Your website is live. Have a look and tell me what to change.\n' > "$T/c14/CONTROL/.speech/turn-07.txt"
  printf '%s | SPEECH-CHECK: clean | file=turn-07.txt\n' "$(stamp 1)" > "$T/c14/CONTROL/LEDGER.md"
  runw "$T/c14"
  ok=0
  if (( RC == 0 )) \
     && ! printf '%s' "$OUT" | "$GREP" -q 'speech-unchecked' \
     && printf '%s' "$OUT" | "$GREP" -q 'speech=ok(1 drafted, all linted)' \
     && ! "$GREP" -q 'DRIFT-ALARM' "$T/c14/CONTROL/LEDGER.md" 2>/dev/null; then ok=1; fi
  report 14 "speech-checked-control" "$ok" "rc=${RC} (want 0); the SPEECH-CHECK line for turn-07.txt silenced the alarm; no DRIFT-ALARM on the ledger; the S-CHECK line carries speech=ok(1 drafted, all linted)"
