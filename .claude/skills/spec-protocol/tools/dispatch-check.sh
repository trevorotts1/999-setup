...
#  11  SEAT-PROBE (RESERVED) — WI-50's gate owns this number; this script
#              implements nothing under it and no other refusal may reuse it,
#              so a caller reading only the exit code never confuses two gates.
...
# contract in full.
#
# The four the row enumerates are 0/3/5/2. Exits 4, 6, 10, 12 and 13 are the
# fail-closed precondition refusals — a missing Parallelism Plan, a missing
# over-engineering check, a conductor off the Opus lane, a missing step-20
# audit gate and a missing ship-guard pass; all five are kept separate from 2
# on purpose, because calling a real refusal a tooling failure would let it
...
run_selftest() {
  ...
  out="$(bash "${SELF}" "${P18}" 1 1 '[Opus x1] publish wave-1' 2>&1)"; rc=$?
  local p18_total
  ...
  [[ "${p18_total}" == "0" ]] || ok=0
  report 44 "no-ship-guard-refused" "${ok}" "rc=${rc} (want 13) for a publish dispatch whose CONTROL/LEDGER.md carries no SHIP-GUARD: rc=0 line; the message names the missing line and the exact path read, and no counter moved (executions_total still ${p18_total}, want 0)"
  ...
  out="$(bash "${SELF}" "${P18}" 1 1 '[Opus x1] publish wave-1' 2>&1)"; rc=$?
  local guard_words
  guard_words="$("${GREP}" -ci guard "${P18}/CONTROL/LEDGER.md")"
  ok=0; [[ "${rc}" == "13" ]] && ok=1
  [[ "${guard_words}" -ge 1 ]] || ok=0
  printf '%s' "${out}" | "${GREP}" -q 'NO-SHIP-GUARD' || ok=0
  report 45 "prose-guard-still-refused" "${ok}" "rc=${rc} (want 13) on a ledger whose own control count says ${guard_words} line(s) contain the word 'guard' (want >= 1) — the gate matches the LINE SHAPE with rc=0, so a word-search implementation passes leg (a) and fails this one"
  ...
  out="$(bash "${SELF}" "${P18}" 1 1 '[Opus x1] publish wave-1' 2>&1)"; rc=$?
  p18_total="$(read_state_total "${P18}/CONTROL/project_state.json")"
  ok=0; [[ "${rc}" == "0" && "${p18_total}" == "1" ]] && ok=1
  report 46 "ship-guard-line-allows" "${ok}" "rc=${rc} (want 0) on the SAME fixture with 'SHIP-GUARD: rc=0 checks=14 at=…' added and nothing else changed; executions_total 0 -> ${p18_total} (want 1). This half is what proves exit 13 is a fact about the missing line and not a class-wide refusal of publish dispatches"
  ...
  local P18B="${T}/proj-shipguard-rc"
  mkdir -p "${P18B}/CONTROL"
  printf 'CLIENT_CAP=10\n' > "${P18B}/CAPACITY-LEDGER.md"
  printf '## Parallelism Plan\n\nwave 1.\n' > "${P18B}/CONTROL/EXECUTION-PLAN.md"
  write_state "${P18B}/CONTROL/project_state.json" 0 200 0 2000
  printf 'CONDUCTOR-SEAT: expected=opus resolved=opus launcher=claude-nine source=session-env\n' > "${P18B}/CONTROL/LEDGER.md"
  printf '2026-09-08T00:05:00Z | AUDIT-GATE | cycle=1 | halt=0 harm=0 scope=0 carry=0 | verdict=PASS\n' >> "${P18B}/CONTROL/LEDGER.md"
  printf '2026-09-08T00:09:00Z | SHIP-GUARD: rc=01 checks=14 at=2026-09-08T00:09:00Z\n' >> "${P18B}/CONTROL/LEDGER.md"
  out="$(bash "${SELF}" "${P18B}" 1 1 '[Opus x1] publish wave-1' 2>&1)"; rc=$?
  ok=0; [[ "${rc}" == "13" ]] && ok=1
  report 47 "ship-guard-rc01-still-refused" "${ok}" "rc=${rc} (want 13) for a publish dispatch whose only guard line reads rc=01 — the rc=0 class is closed on the right, so a suffixed value is not the clean verdict"
  ...
  report 48 "non-publish-not-ship-gated" "${ok}" "rc=${rc} (want 0) for a BUILD dispatch on a ledger with no SHIP-GUARD line — the guard gate is a PUBLISH-phase refusal, so it must not fire here: ${out}"

  # (f) phase=publish names the phase even when the label does not: the
  #     refusal fires on the NAMING, not on one spelling of it.
  local P18C="${T}/proj-shipguard-phase"
  mkdir -p "${P18C}/CONTROL"
  printf 'CLIENT_CAP=10\n' > "${P18C}/CAPACITY-LEDGER.md"
  printf '## Parallelism Plan\n\nwave 1.\n' > "${P18C}/CONTROL/EXECUTION-PLAN.md"
  write_state "${P18C}/CONTROL/project_state.json" 0 200 0 2000
  printf 'CONDUCTOR-SEAT: expected=opus resolved=opus launcher=claude-nine source=session-env\n' > "${P18C}/CONTROL/LEDGER.md"
  printf '2026-09-08T00:05:00Z | AUDIT-GATE | cycle=1 | halt=0 harm=0 scope=0 carry=0 | verdict=PASS\n' >> "${P18C}/CONTROL/LEDGER.md"
  out="$(bash "${SELF}" "${P18C}" 1 1 '[Opus x1] release wave-1' phase=publish 2>&1)"; rc=$?
  ok=0; [[ "${rc}" == "13" ]] && ok=1
  printf '%s' "${out}" | "${GREP}" -q 'NO-SHIP-GUARD' || ok=0
  report 49 "phase-arg-names-publish" "${ok}" "rc=${rc} (want 13) for phase=publish with a label carrying no publish word — the gate reads the phase however the caller named it"
  ...
  local P18D="${T}/proj-shipguard-noledger"
  mkdir -p "${P18D}/CONTROL"
  printf 'CLIENT_CAP=10\n' > "${P18D}/CAPACITY-LEDGER.md"
  printf '## Parallelism Plan\n\nwave 1.\n' > "${P18D}/CONTROL/EXECUTION-PLAN.md"
  write_state "${P18D}/CONTROL/project_state.json" 0 200 0 2000
  out="$(bash "${SELF}" "${P18D}" 1 1 '[Opus x1] publish wave-1' 2>&1)"; rc=$?
  ok=0; [[ "${rc}" == "13" ]] && ok=1
  printf '%s' "${out}" | "${GREP}" -q 'NO-SHIP-GUARD | CONTROL/LEDGER.md carries no SHIP-GUARD: rc=0 line' || ok=0
  if printf '%s' "${out}" | "${GREP}" -q 'DISPATCH-CHECK PASS'; then ok=0; fi
  report 50 "no-ledger-still-refuses" "${ok}" "rc=${rc} (want 13) for a publish dispatch with NO CONTROL/LEDGER.md at all — a missing ledger refuses, and no PASS line is printed: ${out}"
  ...
  local P18E="${T}/proj-shipguard-both"
  mkdir -p "${P18E}/CONTROL"
  printf 'CLIENT_CAP=10\n' > "${P18E}/CAPACITY-LEDGER.md"
  printf '## Parallelism Plan\n\nwave 1.\n' > "${P18E}/CONTROL/EXECUTION-PLAN.md"
  write_state "${P18E}/CONTROL/project_state.json" 0 200 0 2000
  printf 'CONDUCTOR-SEAT: expected=opus resolved=opus launcher=claude-nine source=session-env\n' > "${P18E}/CONTROL/LEDGER.md"
  ...
  out12="$(bash "${SELF}" "${P18E}" 10 10 '[Opus x10] build wave-1' 2>&1)"; rc12=$?
  out13="$(bash "${SELF}" "${P18E}" 1 1 '[Opus x1] publish wave-1' 2>&1)"; rc13=$?
  ...
  report 51 "audit-vs-ship-distinguishable" "${ok}" "rc=${rc12} (want 12) for build and rc=${rc13} (want 13) for publish on the SAME bare ledger — the build message names the AUDIT-GATE line and only that line, the publish message names the SHIP-GUARD: rc=0 line and only that line"
  ...
  printf '\n'
  if (( FAILS == 0 )); then
    printf 'dispatch-check.sh selftest: ALL PASS (52 checks)\n'
    exit 0
  fi
