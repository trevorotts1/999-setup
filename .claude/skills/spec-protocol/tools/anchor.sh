...
EOF
  ...
  #--------------------------------------------------------------------------
  # --- RC-29 (case 22): THE ROUND TRIP. --write-tasks produces a file the
  ...
  : > "$T/c22/CONTROL/LEDGER.md"
  runa --write-tasks "$T/c22"
  local rc22w="$RC" ok22a=0 ok22b=0
  local c22snap="$T/c22/CONTROL/task-graph-snapshot.json"
  if (( rc22w == 0 )) && [[ -f "$c22snap" ]] \
     && "$GREP" -q '"taskId":"U-01"' "$c22snap" 2>/dev/null \
     && "$GREP" -q '"taskId":"U-02"' "$c22snap" 2>/dev/null \
     && "$GREP" -q '"status":"completed"' "$c22snap" 2>/dev/null \
     && "$GREP" -q '"status":"pending"' "$c22snap" 2>/dev/null; then
    ...
    runa "$T/c22" "U-02" --mode reconcile --tasks "$c22snap" --state "$T/c22/CONTROL/project_state.json"
    if printf '%s' "$OUT" | "$GREP" -q 'classes=checked(1,2,3,4)' \
       && "$GREP" -qE 'DRIFT-ALARM \| false-complete' "$T/c22/CONTROL/LEDGER.md" 2>/dev/null; then ok22a=1; fi
  fi
  runa "$T/c22" "IDLE" --mode reconcile --state "$T/c22/CONTROL/project_state.json"
  if printf '%s' "$OUT" | "$GREP" -q 'unchecked(1,2,3,4' \
     && ! printf '%s' "$OUT" | "$GREP" -q 'classes=checked(1,2,3,4)'; then ok22b=1; fi
  ok=0
  if (( ok22a == 1 && ok22b == 1 )); then ok=1; fi
  report 22 "write-tasks-round-trip" "$ok" \
    "write rc=${rc22w} (want 0); round trip reconcile carries classes=checked(1,2,3,4) with a proven false-complete=${ok22a} (the file's taskId/subject/status shape is what the reader parses — a drifted shape fails here, not at file-exists). Control with no --tasks names unchecked(1,2,3,4)=${ok22b} — the checked word is earned, not printed"
