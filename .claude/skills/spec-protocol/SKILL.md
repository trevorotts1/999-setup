...
| **S15 — Media persistence** | Every media work item marked done carries `stored=` and a `perm-url=` whose read-back proof exists (`persist-proof=`), and NO provider-host URL appears in any deliverable, spec document, generated code, or the shipped app. The deny-set is built mechanically and fail-closed from the run's OWN ledger — every URL recorded in a `provider-url=` field, plus the provider result hosts this run actually observed — so it needs no maintained host list and cannot silently rot. At build completion the served-HTML URL-fetch check runs (references/media-pipeline.md 13.12): every image reference extracted from the served HTML is fetched, must return HTTP 200, and must be a permanent GHL media URL in the project-labeled folder — a temp/provider/local URL in the served HTML is a defect of the highest class | A done item without a verified permanent URL reverts to GENERATED-CAPTURED/PERSIST-PENDING and is not merge-eligible; a provider URL found in a deliverable is a defect — replace it with the ledger's permanent URL before the pen; an ASSET-LOST-PAID line missing from the completion report is a defect of the highest class; a unit whose served-HTML check failed (`served-check=fail`) is not merge-eligible |
...
6.5. **Compute the Capacity Ledger (BOTH modes, every launcher — before anything
    ...
    enforced as S15). A media build is not complete until the served-HTML
    URL-fetch check has run on every built page — every image reference
    extracted from the served HTML fetched, HTTP 200 asserted, and each URL
    asserted to be a permanent GHL media URL in the project-labeled folder,
    with any temp/provider/local URL a fail-closed defect
    (`references/media-pipeline.md` 13.12; the check's result is recorded as
    `served-check=` on the MEDIA line). A
