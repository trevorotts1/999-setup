...
2. **CENSUS — by EXTERNAL instruments, in this priority order.** A session cannot
   ...
   - **(a) The teammate's own session transcript — PRIMARY.** Every teammate is a
     full Claude Code session writing `{active config root}/projects/{cwd-slug}/{uuid}.jsonl`,
     and every message line of it carries `"teamName"` and `"agentName"`. The
     transcript existing is the spawn; its tail is what happened. This instrument is
     display-mode-blind, which is the property the inbox artifact lacks. Run the
     procedure in §10 — roster, slug, the known-good control, candidate heads, the
     tail, and the lead-transcript negative branch — and issue no verdict without it.
   - **(a-ii) The on-disk inbox artifact — CORROBORATOR, split-pane display only.**
     `{active config root}/teams/session-{id8}/inboxes/{probe-name}.json`, where
     ...
     session id. Its presence CONFIRMS a spawn; **its absence proves nothing** —
     in-process teammates never create it, and in split-pane mode it is cleared to
     `[]` on delivery. **No negative verdict may cite it.** A read error on the
     directory is an instrument failure (`ls` rc ≥ 2), never an absence.
...
- **Confirm each spawn with the PRIMARY instrument — the commander's own session
  transcript** (§10): resolve the config root, read the roster, compute the slug, run
  the known-good control on the lead's transcript, and identify the commander's
  `{uuid}.jsonl` by the `teamName` + `agentName` pair in its head. **The transcript
  existing is the spawn.** The inbox artifact
  `{active config root}/teams/session-{id8}/inboxes/{name}.json` and, in split-pane
  display mode, an external `tmux list-panes` count increment CORROBORATE it; neither
  is required, because in-process teammates create no inbox and hold no pane.
  `ListAgents` may also corroborate, but **its silence is never evidence of absence** —
  a commander it fails to list is NOT thereby unspawned. A spawn is declared absent
  only through §10 step 8, which closes the question in the LEAD's transcript; a read
  error on any directory (`ls` rc ≥ 2) is an instrument failure, never an absence.
...
1. **Census with the PRIMARY instruments — and assume none are alive.** Take the
   census from the commanders' own session transcripts (§10): for each name in
   `agents.commanders[]`, run §10's procedure — control first, then the transcript's
   tail, whose last assistant line carries the resolved model, the final output and any
   failure text. A commander is DEAD only when §10 says so, and after a crash that is
   the normal case, not an anomaly. The inbox artifacts under
   `{active config root}/teams/session-{id8}/inboxes/` and an external
   `tmux list-panes` count in split-pane mode CORROBORATE the census and never decide
   it — an in-process commander has neither and is not thereby dead.
   **`ListAgents` may also CORROBORATE; its silence is never evidence of absence.**
   This matters in BOTH directions here: because there is exactly one team per session,
   declaring a still-live commander DEAD on an omission alone would re-spawn a name
   that already exists. A read error on any directory (`ls` rc ≥ 2) is an instrument
...
`~/.claude/tasks/{team-name}/`. They answer one question — "did the message
actually land?" — and they answer no other: a mailbox is a delivery diagnostic and a
split-pane-only corroborator, so **no verdict about whether a teammate lives, died or
ever started may cite these paths.** That verdict comes from the transcript
instrument in §10. They are **never hand-edited, never deleted, and never cleaned
up**; modifying another session's live runtime state is forbidden by the safety
envelope without exception.

---

## 10. THE TEAMMATE TRANSCRIPT — the primary liveness instrument

**This section is the single owner of teammate-liveness verification; §3 stage C step 2(a), §4's spawn confirmation and §6 step 1 all run their verdicts through it.** The reason the transcript and not the mailbox is the instrument: the inbox artifact is created **only by the split-pane backends**. In-process teammates never create it, and in-process has been the documented default display mode since v2.1.179. A doctrine that reads inbox absence as teammate absence therefore reads **every default-mode team as dead**. Proven on the authoring box, 2026-08-12: two real teams (`~/.claude/teams/session-1283dd9a`, `~/.claude-nine/teams/session-d97558f6`) held only `config.json`, `ls .../inboxes` returned "No such file or directory" — and an in-process teammate in a third team was demonstrably alive and answering the whole time.
