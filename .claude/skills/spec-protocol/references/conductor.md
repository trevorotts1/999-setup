# Spec Protocol — the conductor's reference (sections 6–14)

Read this file at **step 6.5**, before the first dispatch, and keep it for the rest of the run.
It holds what `SKILL.md` sections 6–14 held before 1.25.0, under the SAME section numbers, so a
...
When the repository receipt is `source=local-only`, the backup clause is replaced by the
truth: the work is saved on this computer and is not yet online.
