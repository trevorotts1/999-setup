...
- **The skill's own artifacts — written silently, backup path recorded.** `~/.claude/statusline-command.sh`
  carrying the `SPEC-PROTOCOL-STATUSLINE` marker, the `autoCompact*` keys in each config root's
  `settings.json`, and the companion skills the bootstrap installs are all THIS skill's output. Each is
  installed or regenerated by its own installer, never edited in place, backed up FIRST, and the backup
...
- **The distinction is the FILE, and NEVER who is at the keyboard.** Which branch applies is settled by
  ...
  own artifacts, and are never explained to a client (`references/audience.md` §7).
