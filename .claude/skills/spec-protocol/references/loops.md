...
| **The cron half** | The crontab line step 3 writes and announces — `tools/watch-tick.sh --arm <project>` writes it idempotently the moment `CONTROL/` exists (0 armed, 3 already present, 2 `crontab` unavailable with the degradation named), and step 21 PROVES it ran rather than arming it — `*/5 * * * * bash <skill>/tools/watch-tick.sh <project> --log <project>/CONTROL/watch-tick.log` (`tools/watch-tick.sh <project> --cron-line` prints it). This half NEVER depends on the model: it keeps ticking through a compaction, a crashed session, a context reset and a sleeping operator, and it dispatches nothing, because a script cannot call session tools. It proves the state and writes it down. |
