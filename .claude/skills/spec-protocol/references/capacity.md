...
the floor on small plans. **Usable = ceiling − reserve, where the reserve is a
quarter of the ceiling or two slots, WHICHEVER IS LARGER** (a tiny plan keeps its
...
it derives from, and cites the GOVERNING NUMBER the ledger computed — never a raw
provider ceiling, never a recalled figure. A dispatch citing no ledger or citing
a raw ceiling as its width basis is a defect the swarm watch (the S-checks in
SKILL.md RULE 5) flags."**
...
**Agent Team line:** 5 persistent occupants are noise against 500 — the full
shape is unchanged.
