...
| `tools/watch-tick.sh` | The five-minute state of the swarm: it reconciles, counts runnable units and open dispatch rows, and checks S2, S3, S5, S6 and S13 | `*/5 * * * * bash <skill>/tools/watch-tick.sh <project> >> <project>/CONTROL/watch-tick.log 2>&1`, written and announced at step 21 | 0 clean (one `S-CHECK \| violations=0 \| runnable=<n> open=<n> trees=<n>` line); 3 findings, one `ACTION\|<verb>\|<target>\|<evidence>` line each; 4 `CONTROL/TERMINAL-DRIFT.flag` exists — nothing dispatches; 2 broken instrument, never an all-clear | `bash tools/watch-tick.sh --selftest` — ten fixtures, read-only | `node scripts/common/watch-tick.mjs` |
...
| Owner | Standards |
|---|---|
| `tools/watch-tick.sh` (every five minutes, cron half) | S2, S3, S5, S6, S13 |
